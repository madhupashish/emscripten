...
function runJSify() {
  ...
  function handleI64Signatures(symbol, snippet, sig) {
    ...
    return modifyJSFunction(snippet, (args, body, async_) => {
      ...
      for (let i = 1; i < sig.length; i++) {
        ...
        if (WASM_BIGINT) {
          if (sig[i] == 'p' || sig[i] == 'j') {
            argConvertions += `  ${i64ParamToI53(name)}\n`;
          }
        } else {
          if (sig[i] == 'j') {
            argConvertions += `  ${i64ParamToI53(name)}\n`;
            newArgs.push(declareI64Param(name));
          } else {
            newArgs.push(name);
          }
        }
...
function(${args}) {
  ...
  return ret;
    ...
    if (!LibraryManager.library[symbol + '__bigint']) {
      if (sig && (sig.includes('j') || (MEMORY64 && sig.includes('p')))) {
        snippet = handleI64Signatures(symbol, snippet, sig, deps);
